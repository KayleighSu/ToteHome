...
export type CloudItem = { id: string; name: string; quantity: string; notes: string; tags?: string[]; stored?: boolean; image?: string; imagePath?: string };
...
export async function saveCloudTote(userId: string, tote: CloudTote) {
  ...
  for (const item of tote.items) {
    ...
    const itemRecord = { tote_id: toteId, name: item.name, quantity: item.quantity, notes: item.notes || null, tags: item.tags || [], image_url: itemPath || null, is_stored: item.stored !== false };
    if (isUuid(item.id)) {
