...
import { acceptCloudInvite, createCloudHousehold, createCloudInvite, loadCloudInventory, saveCloudTote } from './src/cloud';
...
export default function App() {
  ...
  if (selected) return <ToteDetail tote={selected} household={households.find(h => h.id === selected.householdId) || house} onBack={() => setSelected(null)} onSave={saveTote} onLabel={() => setLabel(selected)} label={label} closeLabel={() => setLabel(null)} />;
...
function ToteDetail({ tote, household, onBack, onSave, onLabel, label, closeLabel }: { tote: Tote; household: Household; onBack: () => void; onSave: (t: Tote) => void; onLabel: () => void; label: Tote | null; closeLabel: () => void }) {
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark" /><View style={s.detailHeader}><Pressable style={s.iconButton} onPress={onBack}><Ionicons name="arrow-back" size={23} color={C.ink} /></Pressable><Text style={s.headerTitle}>Tote details</Text><Pressable style={s.iconButton} onPress={onLabel}><Ionicons name="qr-code-outline" size={22} color={C.teal} /></Pressable></View><ScrollView contentContainerStyle={s.detailScroll}>
    ...
    {!tote.items.length && <Text style={s.emptyItems}>Nothing listed yet. Add the first item so it can be found later.</Text>}<Pressable style={s.addItem} onPress={() => setAdding(true)}><Ionicons name="add-circle-outline" size={21} color={C.teal} /><Text style={s.addItemText}>Add an item</Text></Pressable></ScrollView><AddItem visible={adding} close={() => setAdding(false)} add={item => { onSave({ ...tote, items: [...tote.items, item], updatedAt: 'Today' }); setAdding(false); }} /><LabelSheet tote={label} household={household} close={closeLabel} /></SafeAreaView>;
}

function Settings({ household, email, signOut, deleteAccount }: { household: Household; email: string; signOut: () => void; deleteAccount?: () => void }) {
  ...
  return <ScrollView contentContainerStyle={s.scroll}><Text style={s.pageTitle}>Household</Text><Text style={s.subtitle}>Keep your spaces and people organized.</Text><View style={s.profile}><View style={[s.avatar, { backgroundColor: household.color }]}><Text style={s.avatarText}>{household.name[0]}</Text></View><View style={{ flex: 1 }}><Text style={s.profileTitle}>{household.name}</Text><Text style={s.profileMeta}>{household.members} member · Private</Text></View><Ionicons name="chevron-forward" size={20} color="#A9B3B0" /></View><Text style={s.settingsLabel}>ORGANIZATION</Text><Setting icon="location-outline" title="Locations" sub="Attic, garage, basement and more" /><Setting icon="people-outline" title="Members" sub="Invite and manage household access" /><Setting icon="pricetags-outline" title="Label preferences" sub="Paper size, margins and label dimensions" /><Text style={s.settingsLabel}>ACCOUNT</Text><Setting icon="cloud-done-outline" title="Sync status" sub={isCloudConfigured ? 'Account connected' : 'Cloud connection required'} /><Setting icon="person-circle-outline" title="Profile" sub={email} /><Pressable style={s.signOutButton} onPress={signOut}><Ionicons name="log-out-outline" size={19} color={C.coral} /><Text style={s.signOutText}>Sign out</Text></Pressable>{deleteAccount && <Pressable style={s.deleteAccountButton} onPress={confirmDelete}><Ionicons name="trash-outline" size={18} color="#A73D32" /><Text style={s.deleteAccountText}>Delete account permanently</Text></Pressable>}</ScrollView>;
}
function Setting({ icon, title, sub }: { icon: keyof typeof Ionicons.glyphMap; title: string; sub: string }) { return <View style={s.setting}><View style={s.settingIcon}><Ionicons name={icon} size={21} color={C.teal} /></View><View style={{ flex: 1 }}><Text style={s.settingTitle}>{title}</Text><Text style={s.settingSub}>{sub}</Text></View><Ionicons name="chevron-forward" size={18} color="#A9B3B0" /></View>; }
...
function LabelSheet({ tote, household, close }: { tote: Tote | null; household: Household; close: () => void }) {
  if (!tote) return null;
  const active = tote;
    ...
    try {
      ...
      const html = `<!doctype html><html><head><meta charset="utf-8"><title>Tote ${active.number} Labels</title><style>
        ...
        * { box-sizing: border-box; }
        ...
        .sheet { display: flex; flex-direction: column; align-items: center; gap: .35in; }
        .label { position: relative; width: 7.55in; height: 3.25in; border: 3px solid #18332f; border-radius: .18in; padding: .3in; display: grid; grid-template-columns: 1.55in 1fr 2.3in; align-items: center; gap: .28in; overflow: hidden; page-break-inside: avoid; }
        ...
        .number { font-size: 62pt; line-height: .95; font-weight: 900; }
        .title { font-size: 21pt; line-height: 1.05; font-weight: 900; margin-bottom: .2in; }
        .location { font-size: 13pt; line-height: 1.35; font-weight: 700; color: #52645f; }
        .qr { display: block; width: 2.15in; height: 2.15in; justify-self: end; image-rendering: pixelated; }
