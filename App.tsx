...
import { acceptCloudInvite, createCloudHousehold, createCloudInvite, createCloudInviteForUser, deleteCloudItem, loadCloudInventory, loadCloudProfile, loadCloudSuggestions, loadHouseholdMembers, replaceCloudPhoto, saveCloudProfile, saveCloudTote, searchToteHomePeople, setSuggestionStatus, submitCloudSuggestion, type CloudMember, type CloudPerson, type CloudSuggestion } from './src/cloud';

type Household = { id: string; name: string; color: string; members: number; role?: string };
type Item = { id: string; name: string; quantity: string; notes: string; tags?: string[]; stored?: boolean; image?: string; imagePath?: string };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((event, next) => { setSession(next); if (event === 'PASSWORD_RECOVERY') setPasswordRecovery(true); });
  ...
  if (passwordRecovery && session) return <ResetPasswordScreen done={() => setPasswordRecovery(false)} />;
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark" /><View style={s.app}>
    ...
    <View style={s.content}>
      {tab === 'home' && <Home household={house} totes={visibleTotes} locationOrder={locationOrder} reorder={async order => { setLocationOrder(order); if (identity) await AsyncStorage.setItem(`totehome-location-order-${identity}-${householdId}`, JSON.stringify(order)); }} open={setSelected} add={() => setAddTote(true)} suggest={() => setSuggestionOpen(true)} />}
    ...
    <SuggestionSheet visible={suggestionOpen} close={() => setSuggestionOpen(false)} submit={session ? message => submitCloudSuggestion(session.user.id, profile.name, profile.username, message) : undefined} />
  </SafeAreaView>;
}
...
function authReturnUrl() { return Platform.OS === 'web' ? new URL(window.location.pathname.startsWith('/ToteHome') ? '/ToteHome/' : '/', window.location.origin).toString() : 'totehome://auth/callback'; }
...
function AuthScreen({ onPreview }: { onPreview: () => void }) {
  ...
  async function submit() {
    if (!supabase) return;
    if (!email.trim() || password.length < 8) return Alert.alert('Check your details', 'Enter an email and a password with at least 8 characters.');
    setBusy(true);
    try {
      if (mode === 'signup') {
  ...
  async function forgotPassword() { if (!supabase || !email.trim()) return Alert.alert('Enter your email', 'Type your account email first, then tap Forgot password.'); setBusy(true); const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: authReturnUrl() }); setBusy(false); if (error) Alert.alert('Could not send reset email', error.message); else setResetSent(true); }
  ...
  return <SafeAreaView style={s.authSafe}><StatusBar style="light" /><ScrollView contentContainerStyle={s.authScroll} keyboardShouldPersistTaps="handled"><View style={s.authBrand}><View style={s.logoLarge}><Ionicons name="cube" size={30} color="#fff" /></View><Text style={s.authBrandName}>ToteHome</Text><Text style={s.authTagline}>Know exactly where everything lives.</Text></View><View style={s.authCard}><Text style={s.authTitle}>{resetSent ? 'Check your email' : mode === 'signup' ? 'Create your account' : 'Welcome back'}</Text><Text style={s.authSubtitle}>{resetSent ? `We sent a password-reset link to ${email.trim()}.` : mode === 'signup' ? 'Start with an empty household and organize it your way.' : 'Sign in after verifying your email.'}</Text>{resetSent ? <Pressable style={s.fullPrimary} onPress={() => setResetSent(false)}><Text style={s.primaryText}>Back to sign in</Text></Pressable> : <>{mode === 'signup' && <Field label="YOUR NAME" value={name} change={setName} placeholder="Your name" />}<Field label="EMAIL" value={email} change={setEmail} placeholder="you@example.com" /><Field label="PASSWORD" value={password} change={setPassword} placeholder="At least 8 characters" secure /><Pressable style={[s.fullPrimary, busy && { opacity: .6 }]} disabled={busy || !isCloudConfigured} onPress={submit}><Text style={s.primaryText}>{busy ? 'Please wait…' : mode === 'signup' ? 'Create account' : 'Sign in'}</Text></Pressable>{mode === 'signin' && <Pressable style={s.forgotButton} onPress={forgotPassword}><Text style={s.authSwitchText}>Forgot password?</Text></Pressable>}{!isCloudConfigured && <View style={s.cloudNotice}><Ionicons name="cloud-offline-outline" size={19} color={C.coral} /><Text style={s.cloudNoticeText}>Cloud credentials still need to be added before real accounts can connect.</Text></View>}<Pressable style={s.authSwitch} onPress={() => setMode(mode === 'signup' ? 'signin' : 'signup')}><Text style={s.authSwitchText}>{mode === 'signup' ? 'Already verified? Sign in' : 'New to ToteHome? Create an account'}</Text></Pressable><Pressable style={s.previewButton} onPress={onPreview}><Text style={s.previewButtonText}>Preview without an account</Text></Pressable></>}</View></ScrollView></SafeAreaView>;
}

function ResetPasswordScreen({ done }: { done: () => void }) { const [password, setPassword] = useState(''), [confirm, setConfirm] = useState(''), [busy, setBusy] = useState(false); async function save() { if (!supabase || password.length < 8) return Alert.alert('Use at least 8 characters'); if (password !== confirm) return Alert.alert('Passwords do not match'); setBusy(true); const { error } = await supabase.auth.updateUser({ password }); setBusy(false); if (error) return Alert.alert('Could not update password', error.message); Alert.alert('Password updated', 'You can now use your new password.'); done(); } return <SafeAreaView style={s.authSafe}><ScrollView contentContainerStyle={s.authScroll}><View style={s.authCard}><View style={s.emailSuccessIcon}><Ionicons name="key-outline" size={34} color={C.teal} /></View><Text style={[s.authTitle,{textAlign:'center'}]}>Choose a new password</Text><Text style={s.confirmEmail}>Use at least 8 characters and avoid reusing a password from another account.</Text><Field label="NEW PASSWORD" value={password} change={setPassword} placeholder="At least 8 characters" secure /><Field label="CONFIRM PASSWORD" value={confirm} change={setConfirm} placeholder="Type it again" secure /><Pressable style={[s.fullPrimary,busy&&{opacity:.6}]} disabled={busy} onPress={save}><Text style={s.primaryText}>{busy?'Saving…':'Update password'}</Text></Pressable></View></ScrollView></SafeAreaView>; }
...
function Home({ household, totes, locationOrder, reorder, open, add, suggest }: { household: Household; totes: Tote[]; locationOrder: string[]; reorder: (order: string[]) => void; open: (t: Tote) => void; add: () => void; suggest: () => void }) {
  ...
  return <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}><View style={s.hero}><Text style={s.eyebrow}>EVERYTHING IN ITS PLACE</Text><Text style={s.heroTitle}>Find it without{`\n`}opening a single tote.</Text><Text style={s.heroText}>{totes.length} totes and {items} items organized at {household.name.toLowerCase()}.</Text><View style={s.heroActions}><Pressable style={s.primary} onPress={add}><Ionicons name="add" size={21} color="#fff" /><Text style={s.primaryText}>Add a tote</Text></Pressable><Pressable style={s.suggestHome} onPress={suggest}><Ionicons name="bulb-outline" size={18} color="#fff" /><Text style={s.suggestHomeText}>Suggest an idea</Text></Pressable></View></View>
...
function Search({ totes, households, open }: { totes: Tote[]; households: Household[]; open: (t: Tote) => void }) {
  const [query, setQuery] = useState(''); const results = useMemo(() => { const q = query.trim().toLowerCase(); return q ? totes.flatMap(tote => tote.items.filter(i => `${i.name} ${i.notes} ${(i.tags || []).join(' ')}`.toLowerCase().includes(q)).map(item => ({ tote, item }))) : []; }, [query, totes]);
...
function ToteDetail({ tote, household, householdTotes, onBack, onSave, onArchiveItem, onDeleteItem, onMoveItem, onLabel, label, closeLabel, labelSize }: { tote: Tote; household: Household; householdTotes: Tote[]; onBack: () => void; onSave: (t: Tote) => void; onArchiveItem: (item: Item) => Promise<void>; onDeleteItem: (item: Item) => Promise<void>; onMoveItem: (item: Item, targetId: string) => Promise<void>; onLabel: () => void; label: Tote | null; closeLabel: () => void; labelSize: LabelSize }) {
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark" /><View style={s.detailHeader}><Pressable style={s.iconButton} onPress={onBack}><Ionicons name="arrow-back" size={23} color={C.ink} /></Pressable><Text style={s.headerTitle}>Tote details</Text><View style={s.headerActions}><Pressable style={s.iconButton} onPress={() => setEditingTote(true)}><Ionicons name="create-outline" size={21} color={C.teal} /></Pressable><Pressable style={s.iconButton} onPress={onLabel}><Ionicons name="qr-code-outline" size={22} color={C.teal} /></Pressable></View></View><ScrollView contentContainerStyle={s.detailScroll}>
    ...
    {storedItems.map(i => <Pressable key={i.id} style={s.item} onPress={() => setEditing(i)}>{i.image ? <Image source={{ uri: i.image }} style={s.itemImage} /> : <View style={s.check}><Ionicons name="checkmark" size={14} color={C.teal} /></View>}<View style={{ flex: 1 }}><Text style={s.itemName}>{i.name}</Text>{i.notes ? <Text style={s.itemNotes}>{i.notes}</Text> : null}{!!i.tags?.length && <TagList tags={i.tags} compact />}</View><Text style={s.quantity}>{i.quantity}</Text><Ionicons name="create-outline" size={18} color={C.teal} /></Pressable>)}
    {!storedItems.length && <Text style={s.emptyItems}>Nothing listed yet. Add the first item so it can be found later.</Text>}<Pressable style={s.addItem} onPress={() => setAdding(true)}><Ionicons name="add-circle-outline" size={21} color={C.teal} /><Text style={s.addItemText}>Add an item</Text></Pressable></ScrollView><AddItem visible={adding} suggestions={Array.from(new Set(householdTotes.flatMap(t => t.items.flatMap(i => i.tags || []))))} close={() => setAdding(false)} add={item => { onSave({ ...tote, items: [...tote.items, item], updatedAt: 'Today' }); setAdding(false); }} /><EditTote visible={editingTote} tote={tote} close={() => setEditingTote(false)} save={updated => { onSave(updated); setEditingTote(false); }} /><EditItem visible={!!editing} item={editing} currentTote={tote} totes={householdTotes} close={() => setEditing(null)} save={item => { onSave({ ...tote, items: tote.items.map(i => i.id === item.id ? item : i), updatedAt: 'Today' }); setEditing(null); }} archive={async item => { try { await onArchiveItem(item); setEditing(null); } catch (e: any) { Alert.alert('Could not save seasonal item', e.message); } }} move={async (item, targetId) => { try { await onMoveItem({ ...item, stored: true }, targetId); setEditing(null); } catch (e: any) { Alert.alert('Could not move item', e.message); } }} remove={async item => { try { await onDeleteItem(item); setEditing(null); } catch (e: any) { Alert.alert('Could not delete item', e.message); } }} /><LabelSheet tote={label} household={household} close={closeLabel} size={labelSize} /></SafeAreaView>;
}

function Settings({ household, totes, email, profile, labelSize, setLabelSize, openTote, sendSuggestion, suggestionAdmin, invite, searchPeople, invitePerson, loadMembers, saveProfile, signOut, deleteAccount }: { household: Household; totes: Tote[]; email: string; profile: Profile; labelSize: LabelSize; setLabelSize: (size: LabelSize) => void; openTote: (t: Tote) => void; sendSuggestion: () => void; suggestionAdmin: boolean; invite?: (email: string) => Promise<void>; searchPeople?: (query: string) => Promise<CloudPerson[]>; invitePerson?: (userId: string) => Promise<void>; loadMembers?: () => Promise<CloudMember[]>; saveProfile?: (name: string, username: string, discoverable: boolean, image?: string) => Promise<void>; signOut: () => void; deleteAccount?: () => void }) {
  const [panel, setPanel] = useState<'locations' | 'members' | 'labels' | 'profile' | 'inbox' | null>(null);
...
function AddItem({ visible, suggestions, close, add }: { visible: boolean; suggestions: string[]; close: () => void; add: (i: Item) => void }) {
  const [name, setName] = useState(''), [quantity, setQuantity] = useState('1'), [notes, setNotes] = useState(''), [tags, setTags] = useState<string[]>([]), [image, setImage] = useState<string>();
  function submit() { if (!name.trim()) return Alert.alert('What is the item called?'); add({ id: `item-${Date.now()}`, name: name.trim(), quantity: quantity.trim() || '1', notes: notes.trim(), tags, image }); setName(''); setQuantity('1'); setNotes(''); setTags([]); setImage(undefined); }
  async function pick() { const chosen = await choosePhoto(); if (chosen) setImage(chosen); }
  return <Sheet visible={visible} close={close} title="Add an item"><Field label="ITEM NAME" value={name} change={setName} placeholder="e.g. Extension cords" /><Field label="QUANTITY" value={quantity} change={setQuantity} placeholder="1" /><Field label="NOTES (OPTIONAL)" value={notes} change={setNotes} placeholder="Color, size, condition..." /><TagPicker tags={tags} suggestions={suggestions} change={setTags} /><PhotoPicker image={image} pick={pick} remove={() => setImage(undefined)} label="ITEM PHOTO (OPTIONAL)" /><Primary label="Add item" press={submit} /></Sheet>;
}
...
function EditItem({ visible, item, currentTote, totes, close, save, archive, move, remove }: { visible: boolean; item: Item | null; currentTote: Tote; totes: Tote[]; close: () => void; save: (item: Item) => void; archive: (item: Item) => void; move: (item: Item, targetId: string) => Promise<void>; remove: (item: Item) => Promise<void> }) {
  const [name, setName] = useState(''), [quantity, setQuantity] = useState('1'), [notes, setNotes] = useState(''), [tags, setTags] = useState<string[]>([]), [image, setImage] = useState<string>(), [target, setTarget] = useState(''), [busy, setBusy] = useState(false);
  useEffect(() => { if (item) { setName(item.name); setQuantity(item.quantity); setNotes(item.notes); setTags(item.tags || []); setImage(item.image); setTarget(''); } }, [item?.id, visible]);
  if (!item) return null;
  ...
  const updated = { ...item, name: name.trim() || item.name, quantity: quantity.trim() || '1', notes: notes.trim(), tags, image };
  const suggestions = Array.from(new Set(totes.flatMap(t => t.items.flatMap(i => i.tags || []))));
  return <Sheet visible={visible} close={close} title="Edit item"><Field label="ITEM NAME" value={name} change={setName} placeholder="Item name" /><Field label="QUANTITY" value={quantity} change={setQuantity} placeholder="1" /><Field label="NOTES (OPTIONAL)" value={notes} change={setNotes} placeholder="Color, size, condition..." /><TagPicker tags={tags} suggestions={suggestions} change={setTags} /><PhotoPicker image={image} pick={async () => { const p = await choosePhoto(); if (p) setImage(p); }} remove={() => setImage(undefined)} label="ITEM PHOTO (OPTIONAL)" /><Pressable style={s.fullPrimary} onPress={() => save(updated)}><Text style={s.primaryText}>Save changes</Text></Pressable><Pressable style={s.seasonButton} onPress={() => archive(updated)}><Ionicons name="leaf-outline" size={19} color={C.teal} /><Text style={s.outlineText}>Take out for the season</Text></Pressable>{totes.length > 1 && <><View style={s.divider} /><Text style={s.fieldLabel}>MOVE TO ANOTHER TOTE</Text>{totes.filter(t => t.id !== currentTote.id).map(t => <Pressable key={t.id} style={[s.moveOption, target === t.id && s.sizeOptionOn]} onPress={() => setTarget(t.id)}><Text style={s.settingTitle}>Tote {String(t.number).padStart(2, '0')} · {t.title}</Text>{target === t.id && <Ionicons name="checkmark-circle" size={22} color={C.teal} />}</Pressable>)}<Pressable style={[s.outline, (!target || busy) && { opacity: .45 }]} disabled={!target || busy} onPress={async () => { setBusy(true); await move(updated, target); setBusy(false); }}><Ionicons name="swap-horizontal" size={19} color={C.teal} /><Text style={s.outlineText}>Move item</Text></Pressable></>}<Pressable style={s.deleteItemButton} disabled={busy} onPress={confirmDelete}><Ionicons name="trash-outline" size={18} color="#A73D32" /><Text style={s.deleteAccountText}>Delete this item</Text></Pressable></Sheet>;
}
function TagList({ tags, compact = false }: { tags: string[]; compact?: boolean }) { return <View style={[s.tagList, compact && s.tagListCompact]}>{tags.map(tag => <View key={tag.toLowerCase()} style={s.tag}><Text style={s.tagText}>#{tag}</Text></View>)}</View>; }
function TagPicker({ tags, suggestions, change }: { tags: string[]; suggestions: string[]; change: (tags: string[]) => void }) {
  const [draft, setDraft] = useState('');
  function add(value = draft) { const tag = value.trim().replace(/^#/, ''); if (!tag) return; if (!tags.some(existing => existing.toLowerCase() === tag.toLowerCase())) change([...tags, tag]); setDraft(''); }
  const available = suggestions.filter(tag => !tags.some(existing => existing.toLowerCase() === tag.toLowerCase()));
  return <View style={s.field}><Text style={s.fieldLabel}>TAGS (OPTIONAL)</Text><Text style={s.tagHelp}>Add more than one category, such as Halloween, Decor, or Fragile.</Text><View style={s.tagInputRow}><TextInput style={s.tagInput} value={draft} onChangeText={setDraft} onSubmitEditing={() => add()} placeholder="Type a tag" placeholderTextColor="#9BA5A2" returnKeyType="done" /><Pressable style={[s.tagAdd, !draft.trim() && { opacity: .45 }]} disabled={!draft.trim()} onPress={() => add()}><Ionicons name="add" size={20} color="#fff" /></Pressable></View>{!!tags.length && <View style={s.tagList}>{tags.map(tag => <Pressable key={tag.toLowerCase()} style={s.tagSelected} onPress={() => change(tags.filter(value => value !== tag))}><Text style={s.tagSelectedText}>#{tag}</Text><Ionicons name="close" size={14} color={C.teal} /></Pressable>)}</View>}{!!available.length && <><Text style={s.tagSuggestionLabel}>USED IN THIS HOUSEHOLD</Text><View style={s.tagList}>{available.slice(0, 10).map(tag => <Pressable key={tag.toLowerCase()} style={s.tagSuggestion} onPress={() => add(tag)}><Text style={s.tagText}>+ {tag}</Text></Pressable>)}</View></>}</View>;
}
...
function Field({ label, value, change, placeholder, secure }: { label: string; value: string; change: (v: string) => void; placeholder: string; secure?: boolean }) { return <View style={s.field}><Text style={s.fieldLabel}>{label}</Text><TextInput style={s.input} value={value} onChangeText={change} placeholder={placeholder} placeholderTextColor="#9BA5A2" secureTextEntry={secure} autoCapitalize={secure ? 'none' : undefined} /></View>; }
...
function SuggestionSheet({ visible, close, submit }: { visible: boolean; close: () => void; submit?: (message: string) => Promise<void> }) { const [message, setMessage] = useState(''), [busy, setBusy] = useState(false), [sent, setSent] = useState(false); useEffect(() => { if (visible) { setMessage(''); setSent(false); } }, [visible]); async function send() { if (!submit || message.trim().length < 3) return Alert.alert('Tell us a little more'); setBusy(true); try { await submit(message); setSent(true); } catch (e: any) { Alert.alert('Could not send suggestion', e.message); } finally { setBusy(false); } } return <Sheet visible={visible} close={close} title="Suggest an idea">{sent ? <Info icon="checkmark-circle-outline" title="Suggestion received" body="Thanks! Your idea was added to Kayleigh’s private ToteHome inbox." /> : <><Text style={s.sheetIntro}>What would make ToteHome more useful for you? Your email address is not shown or shared.</Text><TextInput style={s.suggestionInput} multiline value={message} onChangeText={setMessage} placeholder="Describe your idea…" placeholderTextColor="#9BA5A2" textAlignVertical="top" /><Pressable style={[s.fullPrimary, busy && { opacity: .6 }]} disabled={busy} onPress={send}><Ionicons name="send-outline" size={19} color="#fff" /><Text style={s.primaryText}>{busy ? 'Sending…' : 'Send suggestion'}</Text></Pressable></>}</Sheet>; }

function SuggestionInbox({ visible, close }: { visible: boolean; close: () => void }) { const [items, setItems] = useState<CloudSuggestion[]>([]), [loading, setLoading] = useState(false); async function refresh() { setLoading(true); try { setItems(await loadCloudSuggestions()); } catch (e: any) { Alert.alert('Could not load suggestions', e.message); } finally { setLoading(false); } } useEffect(() => { if (visible) refresh(); }, [visible]); return <Sheet visible={visible} close={close} title="Suggestion inbox">{loading ? <Text style={s.printHint}>Loading suggestions…</Text> : items.map(item => <View key={item.id} style={[s.suggestionCard, item.status === 'done' && { opacity: .55 }]}><View style={s.suggestionHead}><View><Text style={s.settingTitle}>{item.name}</Text><Text style={s.settingSub}>{item.username ? `@${item.username} · ` : ''}{item.createdAt}</Text></View><Text style={s.suggestionStatus}>{item.status}</Text></View><Text style={s.suggestionMessage}>{item.message}</Text><View style={s.suggestionActions}>{item.status !== 'reviewed' && <Pressable style={s.inboxButton} onPress={async () => { await setSuggestionStatus(item.id,'reviewed'); refresh(); }}><Text style={s.restoreText}>Reviewed</Text></Pressable>}{item.status !== 'done' && <Pressable style={s.inboxButton} onPress={async () => { await setSuggestionStatus(item.id,'done'); refresh(); }}><Text style={s.restoreText}>Done</Text></Pressable>}</View></View>)}{!loading && !items.length && <Info icon="mail-open-outline" title="Inbox is empty" body="New suggestions will appear here." />}</Sheet>; }
...
const s = StyleSheet.create({
  ...
  forgotButton: { alignItems: 'center', paddingVertical: 10 },
  heroActions: { flexDirection: 'row', alignItems: 'center', gap: 10, flexWrap: 'wrap' }, suggestHome: { marginTop: 20, height: 45, paddingHorizontal: 14, borderRadius: 14, borderWidth: 1, borderColor: '#78908A', flexDirection: 'row', gap: 6, alignItems: 'center' }, suggestHomeText: { color: '#fff', fontSize: 12, fontWeight: '800' }, suggestionInput: { minHeight: 145, backgroundColor: '#fff', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#D7DCD8', color: C.ink, fontSize: 16, marginBottom: 14 }, suggestionCard: { backgroundColor: '#fff', padding: 14, borderRadius: 15, borderWidth: 1, borderColor: C.line, marginBottom: 10 }, suggestionHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, suggestionStatus: { color: C.teal, fontSize: 10, fontWeight: '900', textTransform: 'uppercase' }, suggestionMessage: { color: C.ink, fontSize: 13, lineHeight: 19, marginTop: 12 }, suggestionActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 8, marginTop: 12 }, inboxButton: { height: 32, paddingHorizontal: 11, borderRadius: 10, backgroundColor: C.pale, alignItems: 'center', justifyContent: 'center' },
  tagHelp: { color: C.muted, fontSize: 11, lineHeight: 16, marginBottom: 8 }, tagInputRow: { flexDirection: 'row', gap: 8 }, tagInput: { flex: 1, height: 47, backgroundColor: '#fff', borderRadius: 13, paddingHorizontal: 13, borderWidth: 1, borderColor: '#D7DCD8', color: C.ink, fontSize: 15 }, tagAdd: { width: 47, height: 47, borderRadius: 13, backgroundColor: C.teal, alignItems: 'center', justifyContent: 'center' }, tagList: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 9 }, tagListCompact: { marginTop: 5 }, tag: { backgroundColor: C.pale, borderRadius: 10, paddingHorizontal: 7, paddingVertical: 3 }, tagText: { color: C.teal, fontSize: 10, fontWeight: '800' }, tagSelected: { backgroundColor: C.pale, borderRadius: 11, paddingLeft: 9, paddingRight: 6, paddingVertical: 5, flexDirection: 'row', alignItems: 'center', gap: 4 }, tagSelectedText: { color: C.teal, fontSize: 11, fontWeight: '800' }, tagSuggestionLabel: { color: '#82908B', fontSize: 9, fontWeight: '900', letterSpacing: 1, marginTop: 12 }, tagSuggestion: { borderWidth: 1, borderColor: '#B9D6CE', backgroundColor: '#F7FBF9', borderRadius: 10, paddingHorizontal: 8, paddingVertical: 5 },
});
